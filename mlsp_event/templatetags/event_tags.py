...
from mlsp_event.forms import EventPostForm
from mlsp_event.models import EventPost, EventCategory
...
def event_months(*args):
    """
    Put a list of dates for event posts into the template context.
    """
...
def event_categories(*args):
    """
    Put a list of categories for event posts into the template context.
    """
...
def event_authors(*args):
    """
    Put a list of authors (users) for event posts into the template context.
    """
...
def event_recent_posts(limit=5, tag=None, username=None, category=None):
    """
    Put a list of recently published event posts into the template
    ...
    Usage::
        ...
        {% event_recent_posts 5 as recent_posts %}
        {% event_recent_posts limit=5 tag="django" as recent_posts %}
        {% event_recent_posts limit=5 category="python" as recent_posts %}
        {% event_recent_posts 5 username=admin as recent_posts %}
...
def quick_event(context):
    """
    Admin dashboard tag for the quick event form.
    """
